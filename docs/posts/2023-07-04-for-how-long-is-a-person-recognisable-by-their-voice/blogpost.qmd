...
author:
  - name: Maya Nachesa
    ...
    affiliation: KBLab
    affiliation_url: https://www.kb.se/in-english/research-collaboration/kblab.html1
format: html
...
The data for this project consisted of Riksdagen's parliamentary speeches. In the blog "Finding Speeches in the Riksdag’s debates" [@rekathati2023finding] you can read about how the speeches were segmented and their precise timestamps were determined, and in the blog "RixVox: A Swedish Speech Corpus with 5500 Hours of Speech from Parliamentary Debates" [@rekathati2023rixvox] you can read about the resulting dataset.
...
The data was further divided into train, dev, and test. The test data is created first. I tested how voiceprints age for a total age-gap of 9 years, and so only kept those speakers that were active in each of those years from the start of their presence in parliament. After this, I bucketed the speakers into age-ranges of 5 years. Each bucket contained a maximum of 4 speakers (balanced for gender, unless not possible). After this, I paired up the voiceprints in 4 different manners. They can be first distinguished by whether they compare the same or a different speaker, and then by whether they compare them at the same age or different age. The below table shows what the age-gaps are for each of the combinations.
...
: Data distribution and characteristics per split {#tbl-datachars}
...
![T-SNE plot of test speakers at all lengths](images/T-SNE%20for%20all%20speeches%20at%20all%20speech%20lengths.png){#fig-tsne}
...
::: {#fig-train-acc layout="[52.5,47.5]"}

![Trainig accuracy, FNR, and FPR](images/train_acc_vs_fnr_n_fpr_within_age_within_age_VS_across_speaker_all.svg){#fig-train-acc-fnr-fpr}

![Comparison of train and dev accuracy](images/train_VS_dev_acc_within_age_VS_across_speaker_all_speech_lengths.svg){#fig-train-dev-acc}

Accuracy, FPR, and FNR at different thresholds.
:::

@fig-train-acc-fnr-fpr shows the training accuracy, FNR, and FPR, and @fig-train-dev-acc compares the training and dev accuracy. The vertical dashed line indicates the threshold. The threshold is set at 0.463, and we can see that there is no large difference in accuracy between the train and dev. This gives us confidence that the threshold we set is generalisable.
...
::: {#fig-test-acc layout="[49, 51]"}

![Percentiles (at intervals of 10\%) of cosine similarities for each age-gap, and comparing two different speakers](images/test_within_speaker_across_age_VS_across_speaker_all_speech_lengths_cossim_score.svg){#fig-age-diff-histogram}

![Test accuracy, FNR, and FPR for each age-gap](images/test_acc_vs_fnr_n_fpr_across_age_VS_across_speaker_all_speech_lengths.svg){#fig-age-VS-acc-fnr-fpr}

Effect of age-gap on cosine similarity, accuracy, FNR, and FPR scores.
:::

@fig-age-diff-histogram shows how the cosine similarity scores develop as the age-gap between two voiceprints increases, and also when comparing the voiceprints of two different speakers. In general, the cosine similarity seems to drop as the age-gap increases, but the cosine similarity is much lower when comparing two different speakers. @fig-age-VS-acc-fnr-fpr shows how, as the age-gap between two voiceprints increases, the accuracy drops slightly, and experiences an even sharper drop around the 5-year age-gap. This is characterised by an increase in FNR as the age-gap increases.
...
::: {#fig-start-age-acc layout="[49, 51]"}

![Effect of starting age on same-age accuracy](images/test_bucket_acc_vs_fnr_n_fpr_within_within_age_VS_across_speaker_all_speech_lengths.svg){#fig-bucket-within}

![Effect of starting age on accuracy for a 9 year age-span](images/test_bucket_acc_vs_fnr_n_fpr_across_across_age_VS_across_speaker_all_speech_lengths.svg){#fig-bucket-across}

Effect of starting age on same-age and different-age scores.
:::

@fig-bucket-within shows that speakers in the 29-33 age-range are the easiest to recognise, and older and younger speakers are harder to recognise. However, the difference is not large. @fig-bucket-across uses the different-age group for the same-speaker group. That is, the speaker verification is tested for this entire 9-year age-range for each speaker. We see that accuracy is highest when someone's voiceprint is recorded in the 29-33 age-range and used for the next 9 years, and that it is lower for other age groups. Presumably this could be the case due to younger speakers' voices still changing too much, and older speakers' having begun to change again.

::: {#fig-heatmap-acc}

![](images/test_accuracy_heatmap.svg)

Heatmap of accuracy scores for all speech length comparisons.
:::

@fig-heatmap-acc shows that accuracy is very low when at least one of the two voiceprints in a pair comes from a 1-second long audio. It also shows that accuracy is slightly diminished when the pairs use longer audio.

::: {#fig-heatmap-fnr-fpr layout="[50, 50]"}

![Heatmap of FNR for all speech length comparisons](images/test_fnr_heatmap.svg){#fig-heatmap-fnr}

![Heatmap of FPR for all speech length comparisons](images/test_fpr_heatmap.svg){#fig-heatmap-fpr}

Heatmap of FNR and FPR scores for all speech length comparisons.
:::

@fig-heatmap-fnr and @fig-heatmap-fpr shed some light on the accuracy scores. The very low performance of the short speeches seems to be attributable to a high FNR, meaning that we were much more likely to miss when two voiceprints belonged to the same person. The lower performance for longer audio lengths is attributable to a reduction in FPR: we were more likely to accidentally mark two unrelated speakers as being the same person.
