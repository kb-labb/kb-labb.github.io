...
When you click "knit" on your post's `.Rmd`-file, it will only render and update the contents related to your own post. While this is fine for generating a preview, ultimately will also want to re-knit the `index.Rmd` file in the root directory. This will ensure that your new post gets listed on the front page. 

A second way to ensure that everything is up to date before publishing, is to render the whole website via

```{r, eval=FALSE, echo=TRUE}
rmarkdown::render_site()
```

This will however re-render all previous blog posts as well. It may take some time depending on whether or not a post includes heavy computations. 
...
When run with `echo=TRUE`, `eval=TRUE` and `code_folding=FALSE` the above produces the following result:

```{python, echo=TRUE, eval=TRUE, code_folding=FALSE}
# Dictionaries in an R IDE?!?! Amazing!
my_dict = {"a": 5, "b": 9, "c": 4}
my_key = "c"
print(f"The value for '{my_key}' is: {my_dict[my_key]}.")
