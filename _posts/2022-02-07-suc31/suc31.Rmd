...
The original annotations of SUC are sometimes criticized to be somewhat inconsistent ("farbror _Johan_" and "_farbror Johan_", where _farbror_ in the first example does not belong to the full entity, whereas it does in the second), to contain needlessly specific categories (animal, myth), and a dangerously confusing (for a machine) _other_ category. In some instances one would therefore prefer the tags automatically annotated by the tagger, over the manual annotations.
...
```{r, fig.retina=1, fig.cap="Weights and biases overview over the results from an HPO experiment with the simple lower mix variant of the dataset."}
knitr::include_graphics("images/RS_simple_lowermix_overall.png")
```
...
```{r, fig.retina=1, fig.cap="The validation F1 scores for an HPO session using ASHA, searching hyperparameters for the original lower case variation of the dataset."}
knitr::include_graphics("images/ASHA_original_lower_f1.png")
```
