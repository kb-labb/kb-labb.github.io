...
reticulate::use_condaenv("pytorchenv")
library(reactable)
```
...
<aside>Data download scripts can be found on the following links: 
...
[`get_parallel_data_ted2020.py`](https://github.com/kb-labb/swedish-sbert/blob/main/get_parallel_data_ted2020.py)</aside>
...
```{r multilingual, fig.retina=2, fig.width=6, fig.cap="Student model takes a sentence from a source language and a sentence from a target language and minimizes the mean squared error of each to the teacher model's embedding. Image source from @multilingual-sentence-bert."}
knitr::include_graphics("images/multilingual.jpg") 
```
...
<aside>Code to replicate results: 
[evaluate_sweparaphrase.py](https://github.com/kb-labb/swedish-sbert/blob/main/evaluate_sweparaphrase.py)</aside>
...
<aside>Replicate results: [evaluate_faq.py](https://github.com/kb-labb/swedish-sbert/blob/main/evaluate_faq.py)</aside>
...
<aside>Replicate results: [evaluate_swesat.py](https://github.com/kb-labb/swedish-sbert/blob/main/evaluate_swesat.py)</aside>
...
<aside>Replicate results: [evaluate_supersim.py](https://github.com/kb-labb/swedish-sbert/blob/main/evaluate_supersim.py)</aside>

It appears KB-SBERT is not to recommend for word embeddings. 

## Closing words

KB-SBERT appears to performs well on sentence similarity tasks. Training on parallel corpora using a teacher model seems to produce better results compared to machine translation. However, it still remains to be seen whether the distilled model behaves well when fine-tuned on downstream tasks in Swedish without any English supervision. 
