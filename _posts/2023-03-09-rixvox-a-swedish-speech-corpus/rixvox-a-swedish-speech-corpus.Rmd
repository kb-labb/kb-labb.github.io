...
description: |
  RixVox consists of 5500 hours of speech from parliamentary debates that have been 
  aligned with transcripts from written protocols. RixVox is open and free for 
  everyone to download and use.
  KBLab releases RixVox: a new Swedish Speech Corpus for Automatic Speech Recognition. 
author:
...
The dataset splits were created by sampling speakers until a threshold reaching a certain total duration of speech was reached. For the training set, we randomly sampled speakers until $98\%$ of the total duration of the RixVox dataset was reached ($5384$ hours). For the test and validation set, we randomly sampled speakers until each filled up to a bucket of $1\%$ of the total duration of the entire dataset. 
...
RixVox has [a dataset card](https://huggingface.co/datasets/KBLab/rixvox) on Huggingface, where you can find more details about the dataset, its features, and how to doqnload and use it. You can also preview the first 100 observations of the train, validation and test sets in [the dataset viewer](https://huggingface.co/datasets/KBLab/rixvox/viewer/default/train).
